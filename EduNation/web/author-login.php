  ...
  if(isset($_POST['signup-submit'])) {
    ...
    // If the query has no records => No author with the email exists (New Author)
      ...
      if($check_user_row > 0) {
        ...
        $signupQueryAuthor = "INSERT INTO author (author_name, author_email, author_password) VALUES (?, ?, ?)";
        $stmt = mysqli_prepare($con, $signupQueryAuthor);
        mysqli_stmt_bind_param($stmt, "sss", $signupName, $signupEmail, $strg_pass);
        $author_result = mysqli_stmt_execute($stmt);
